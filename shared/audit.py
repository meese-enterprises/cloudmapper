...
def audit_accessanalyzer(findings, region):
    analyzer_list_json = query_aws(region.account, "accessanalyzer-list-analyzers", region)
...
def audit_iam(findings, region):
    ...
    find_admins_in_account(region, s3_listing_findings, privs_to_look_for=["s3:ListAllMyBuckets"])
...
def audit_rds(findings, region):
    ...
    for instance in json_blob.get("DBInstances", []):
        if instance["PubliclyAccessible"]:
            findings.add(Finding(region, "RDS_PUBLIC_IP", instance["DBInstanceIdentifier"]))
        if instance.get("DBSubnetGroup", {}).get("VpcId", "") == "":
            findings.add(Finding(region, "RDS_VPC_CLASSIC", instance["DBInstanceIdentifier"]))
...
def audit_ecr_repos(findings, region):
    ...
    for repo in json_blob.get("repositories", []):
        ...
        policy = Policy(policy)
        if policy.is_internet_accessible():
            findings.add(Finding(region, "ECR_PUBLIC", name, resource_details=policy_string))
...
def audit_redshift(findings, region):
    ...
    for cluster in json_blob.get("Clusters", []):
        if cluster["PubliclyAccessible"]:
            findings.add(Finding(region, "REDSHIFT_PUBLIC_IP", cluster["ClusterIdentifier"]))
